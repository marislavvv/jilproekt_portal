...
function ProjectsPage({ user }) {
    ...
    const fetchProjects = async () => {
        ...
        setError('');
        try {
            const currentRes = await axios.get(`${API_URL}/projects/current`);
            setCurrentProjects(currentRes.data);

            const completedRes = await axios.get(`${API_URL}/projects/completed`);
    ...
    const handleAddProject = async (e) => {
        ...
        if (projectImage) {
            formData.append('projectImage', projectImage); // Имя поля должно совпадать с Multer
        }
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
                    {currentProjects.map((project) => (
                        <div key={project._id} style={projectCardStyle}>
                            <h4>{project.title}</h4>
                            {project.imageUrl && (
                                <img src={`http://localhost:5000${project.imageUrl}`} alt={project.title} style={imageStyle} />
                            )}
                            ...
                            {isManagerOrAdmin && (
                                <div style={{ marginTop: 'auto', paddingTop: '10px', borderTop: '1px dashed #eee' }}>
                                    ...
                                    {(user.role === 'admin') && ( // Только администратор может удалять
                                        <button
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
                    {completedProjects.map((project) => (
                        <div key={project._id} style={{ ...projectCardStyle, opacity: 0.8, backgroundColor: '#e9ecef' }}>
                            ...
                            <p><strong>Отделы:</strong> {project.departments.join(', ')}</p>
                            {isManagerOrAdmin && user.role === 'admin' && ( // Только администратор может удалять
                                <div style={{ marginTop: 'auto', paddingTop: '10px', borderTop: '1px dashed #eee' }}>
                                    <button
                                        onClick={() => handleDeleteProject(project._id)}
                                        style={{ ...buttonStyle, backgroundColor: '#dc3545', color: 'white' }}
